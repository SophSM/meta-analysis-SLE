...
do_DE <- function(dge, rse_gene){
  design <- model.matrix(as.formula(~ 1 + DISEASE), data = colData(rse_gene))
  ...
  de_results <- topTable(eb_results, p.value = 1, coef = ncol(design), number = nrow(rse_gene), confint = T)
...
for(i in 1:length(SRA_projects)){
  ...
  # saving as csv
  write_csv(as.data.frame(DE_result), file = str_c("/mnt/Citosina/amedina/ssalazar/meta/out/Table_Genes_", names(rse_curated[i]), ".csv"))
