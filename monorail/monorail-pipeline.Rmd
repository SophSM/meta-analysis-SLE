...
The script with the lines used to get these indexes and the setup of the `reference` folder are in `monorail/get-human-folder.sh`
...
A `pump`folder is created inside each individual study folder
The `pump-commands.sh` bash script is created using the python script `get-pump-commands.py` specific for each study and can be found inside each study folder in `monorail/SRP*/get-pump-commands.py`.
...
**IMPORTANT** for this pipeline, the `recount-unify_1.0.9.sif` singularity image need to be inside each study's `unify_output` folder, however, only one image needs to be downloaded and can be moved to the different folders with `mv`, alternatively, the full stable location path for the image can be specified in the`hpc_unify_**.sh` script if preferable.

The `hpc_unify_**.sh` scrip contains the commands necessary to run the unify module and is specific for each study, can be found in `monorail/SRP*/hpc_unify_**.sh`. 
...
After the unify module is finished for all studies, the code lines used for the set up of the `human` folder with all outputs as well as the metadata can be found in the beginning of `download-1.Rmd` and is necessary for the further analysis workflow in R.
